...
// Hide/Show Activity Section
function toggleActivitySection(status) {

    const sectionTitles = document.querySelectorAll('h1.section-title');
    for (const title of sectionTitles) {
        if (title.textContent.trim() === 'Activity') {
...
function connectLanyard() {
    ...
    ws.onmessage = event => {
        ...
        // Toggle activity section based on Discord status
        toggleActivitySection(data.discord_status);

        // Only update Discord info if not offline
        if (data.discord_status !== 'offline') {
            ...
            // Hide all activity cards when offline
            const gameCard = document.getElementById("game-card");
            if (gameCard) {
                gameCard.style.display = "none";
                if (gameCard.gameInterval) clearInterval(gameCard.gameInterval);
